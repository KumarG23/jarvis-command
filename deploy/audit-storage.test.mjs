...
test('command storage binds are explicit opt-in and never automatically create host storage', () => {
  ...
  assert.match(override, /source: \/var\/lib\/jarvis-command\/artifacts/);
  assert.match(override, /target: \/var\/lib\/jarvis-command\/artifacts/);
  ...
  assert.doesNotMatch(readFileSync('deploy/app.compose.yaml', 'utf8'), /\/artifacts/);
  ...
  assert.match(env, /^ARTIFACTS_MODE=enabled$/m);
  assert.match(env, /^ARTIFACT_STORAGE_PATH=\/var\/lib\/jarvis-command\/artifacts$/m);
});

test('command storage provisioning refuses unsafe disposable fixtures without repairing them', () => {
  const run = spawnSync('sudo', ['-n', 'python3', 'deploy/audit_storage_test.py'], { encoding: 'utf8', timeout: 15000 });
  assert.equal(run.status, 0, run.stdout + run.stderr);
  const artifacts = spawnSync('sudo', ['-n', 'python3', 'deploy/artifact_storage_test.py'], { encoding: 'utf8', timeout: 15000 });
  assert.equal(artifacts.status, 0, artifacts.stdout + artifacts.stderr);
});
