...
## Deploy 10 trusted recovery roots — local partial, unreviewed

The complete app-only recovery checkpoint is now approved/pushed at
`f95c3508a799218cdd29cfcc79ad92a7e886191d`; the Deploy 9 pending-review wording
below is historical. Deploy 10 is new uncommitted local work, not production cutover.
Both recovery scripts now refuse untrusted backup/state ancestry without chmod or
automatic root creation, pin one opened directory identity, validate rollback files,
and suppress receipts on identity replacement. Shared validation is embedded in
standalone release scripts before checksum staging or direct stdin publication.

Asserting RED reproduced cutover accepting a symlink, association chmod changing
unrelated metadata, trailing-newline rollback normalization, and a failed Docker
rollback inspection incorrectly reporting restoration. Local deployment regressions
pass with nonprivileged UID/anchor substitutions and mocked services. Both scripts
exercise trusted success, mid-step failure, HUP/INT/TERM, root replacement, preserved
state, and refusal without consequential commands. The failure-matrix continuation
adds both packaged rollback adversarial matrices (including every required missing
scalar/artifact, malformed status, symlink/hardlink/FIFO, metadata and identity timing),
helper and packaged acquisition scheduling, and installer-local persistent/one-shot
snapshot-cleanup failures. Asserting regressions fixed late required-artifact checks,
missing/forged status acceptance, insufficient acquisition revalidation, and cleanup
disarming rollback/EXIT retry erasing a retained snapshot. Cleanup failure now restores
the prior rollout, retains recovery state and the remaining snapshot, records explicit
cleanup failure metadata, and suppresses the successful release receipt.

A guarded immutable private-root probe actually exercised UID 0 and anchor `/`,
including both packaged scripts' wrong UID/GID leaf/ancestor/file refusal and pinned
ownership/mode changes. Its checksum readback, zero consequential-command assertions,
sentinel preservation and exact root-directory cleanup are saved separately from the
ordinary-user mock-service tests. No service, container, network or host configuration
was changed. Parent final full gates and exact independent review remain required;
installation/reboot/real-Hermes/desktop/Pixel/live rollback are separate authorized gates.
Evidence: `deploy-10-transactional-roots/failure-matrix-1/IMPLEMENTATION-RESULT.json`
(authoritative final local counts, bindings, limitations and remaining acceptance).
