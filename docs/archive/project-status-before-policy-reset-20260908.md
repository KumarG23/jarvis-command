# Historical project status — policy superseded

Retained evidence, NOT current instructions. General complete-candidate/Astra+Sol/high/review-receipt/parent-repeat mandates were superseded on 2026-09-08 by AGENTS.md. Preserve real security and recovery constraints; old production status may be stale.

# Jarvis Command — current engineering status

Updated: 2026-09-06 (Eastern).

## Current checkpoint — shipped timeline; incomplete v0.3A room candidate

Parent-confirmed shipped timeline checkpoint: `a7c71b45f6a5a853c09be7cc55211b401c27ae3b`, independently approved and publicly verified. Repeated identical messages render once per submission; Copy/reload, live tool completion and applied guidance passed. Evidence: `/home/neal/backups/jarvis-command/continuous-timeline/v3/review/REVIEW-RESULT.json` and `v3/deployment/POST-PUBLIC-VERIFIED.json`. The preceding `public-candidate.json` retains its historical first-attempt old-bundle failure alongside successful rerun fields. These are parent-provided release facts, not new production probes by this implementer.

This worktree, `feat/project-rooms`, contains an uncommitted v0.3A candidate based on that checkpoint. Implemented: authenticated durable bounded room metadata, atomic private-store replacement beside (not inside) the audit ledger, named create/read flow, exact Command-owned conversation lookup/association independent of the recent-session list, per-room last conversation, reload/resume UI, All sessions, metadata-only project details, and honest adapter-label wording. Existing LiveRoom/useLiveTurn behavior remains the conversation/supervision authority. No repository/note content is read or applied as execution configuration. The narrow exact-session GET requires the matching command-proxy candidate; an app-only rollout is insufficient.

Runtime closure evidence now exists under `/home/neal/backups/jarvis-command/project-rooms/runtime-closure/`. Desktop/phone room lifecycle and recovered-run navigation passed (4 Playwright cases); page/timeline horizontal bounds and composer separation passed. Screenshots were inspected: desktop named navigation is real; phone is usable but the inherited snapshot strip and native conversation select truncate secondary labels. New sidebar navigation and prevention of session recreation after failed association have asserting UI RED/GREEN evidence; 38 focused web tests passed. Exact ID remains available for explicit attachment retry. Room metadata rapid submits are coalesced in the UI; intentional duplicate metadata requests produce separate room IDs server-side, not enterprise idempotency.

Actual compiled OS processes and immutable Docker app/read-proxy/command-proxy images exercised production Access JWT verification against ephemeral synthetic identities/upstream only. Create→link→read→app process/container replacement→exact metadata/session readback passed with two independent HTTP clients, including a linked session absent from exactly twelve bootstrap recents. Denied identity/Origin/custom-header room and association writes caused zero upstream requests; all owned process/container/root cleanup was checked. No host networking, published ports, production services, real credentials, or actual Hermes commands were used. App UID 10001 and the private audit mount remained enforced in the image smoke; narrowly scoped fixture helpers provisioned/removed only the disposable audit leaf.

Storage/route acceptance passed (19 focused tests): injected write/file-fsync/rename failures preserve precommit bytes, directory-fsync failure after rename is explicitly uncertain and poisons the store until restart/readback, invalid modes/hardlinks/schema refuse, and injected file/directory UID mismatches refuse. Owner-negative tests inject stat UID on real handles rather than changing host ownership. Additional acceptance probes passed existing behavior; their initial module-spy setup failure is retained, not presented as production RED.

The runtime worker stopped INCOMPLETE at its finite budget. Parent preserved and verified its full 247-path snapshot, then corrected only two test compilation/lint issues: unsupported Testing Library role options and an inline import type. Production behavior was not changed by these repairs. Final full tests, types/lint/build, production audit, browser/e2e compilation, and a fresh exact-source image smoke are now parent-owned gates, with authoritative results under `/home/neal/backups/jarvis-command/project-rooms/final-closure/`. Earlier successful gates and images are historical, not exact-final evidence. Complete-candidate Sol approval and public release remain separate mandatory gates; neither has occurred for rooms. No project-room commit or production mutation is claimed. This status is intentionally written before final gates/freeze; consult their receipts rather than interpreting this paragraph as approval.

Current evidence and frozen receipt: `/home/neal/backups/jarvis-command/project-rooms/runtime-closure/IMPLEMENTATION-RESULT.json` and `HANDOFF.md`. Earlier evidence in the parent directory remains immutable and historical. The remaining sections are historical and do not override this checkpoint.

## Historical continuous timeline — pre-release local candidate
