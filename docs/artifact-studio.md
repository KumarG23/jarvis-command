...
- The main chat composer accepts pasted, dropped or file-picked PNG/JPEG/GIF/WebP images. Each image is persisted first as a session/project-linked private artifact; the exact immutable artifact version is then resolved server-side and sent to Hermes as multimodal input. A message may carry up to four images totaling 6 MiB. Raw image bytes never enter browser recovery storage or the public run/audit contract.
