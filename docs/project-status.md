...
Production was updated on 2026-09-22 at 10:52 ET from source commit `9bf5aaf862d3f3aa5bd4a9213f841f0145c39da6`. The app and command proxy run `jarvis-command-app:9bf5aaf` and `jarvis-command-command-proxy:9bf5aaf`; the public UI exposed Grok 4.7 and completed an exact `xai-oauth/grok-4.7` medium-effort run with `COMMAND_GROK47_OK`. Pre-release `release.env` copies are retained under `/var/backups/jarvis-command/grok47-20260922T105201-0400/` on both hosts, and the prior image tags remain available for rollback.
