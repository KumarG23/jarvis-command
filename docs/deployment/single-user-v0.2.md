...
For a later attempt after the failed Compose cutover, first independently verify complete baseline recovery on BOTH hosts: original file bytes/owners/modes, old immutable images, service health/enablement, disk/live owned nft table, authenticated read bridge, loopback listeners, unchanged Access/read credentials/tunnel and Android association. Preserve the retained audit ledger; require quiescent command state. A stale failed/not-found command-proxy unit is not a running service and is not authorization to start it.

Only after that recovery is recorded, archive the old `/var/lib/jarvis-command-v02-stage` and `/var/lib/jarvis-command-v02-backup` on EACH host under a fresh, uniquely named root:root 0700 attempt directory on the same `/var/lib` filesystem. Record their recursive file hashes, types, ownership and modes privately before moving; move the two directories intact (no overwrite, merge, re-extraction or permission changes), then compare the same relative inventory under the archive and require both old fixed paths absent. Preserve every byte, including credentials and diagnostics, inside the private archive; never transfer it to public evidence. Stop on mismatch. Only then allocate the new fixed stage in step 2. Do not rerun `apply` against retained failed-attempt state. This is an operator preparation step, not a retry framework.

The external supervisor accepts the two verified Compose representations of the audit bind: Compose 2.40.3 emits `bind: {}`, whereas Compose 5.5.0 retains `create_host_path: false`. The raw storage override must still exactly declare false; absent bind, true, malformed values and extra options remain refused, and the ledger must already exist. Do not upgrade Compose or remove storage guards as a workaround. Before cutover, require the complete isolated supervisor start/health/stop gate on VM113's actual Compose/daemon and bind the supervisor source hash to the reviewed candidate. The Compose compatibility receipt is `/home/neal/backups/jarvis-command/compose-compat-repair/IMPLEMENTATION-RESULT.json`; its synthetic isolated runtime evidence is not live public Access/Hermes acceptance. The app/proxy image source maps must still match; an external-supervisor-only change reuses the exact immutable images.
