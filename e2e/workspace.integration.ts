...
test('edits, filters and resumes rooms with readable non-overlapping navigation', async ({ browser }, info) => {
  ...
  try {
    ...
    const mobile = info.project.name.startsWith('phone');
    const openNavigation = async () => { if (mobile) await page.getByRole('button', { name: 'Open chat navigation' }).click(); };
    await openNavigation();
    await page.getByRole('button', { name, exact: true }).click();
    if (mobile) await page.getByRole('button', { name: 'Close chat navigation' }).click();
    ...
    const label = page.getByLabel('Selected project', { exact: true });
    ...
    await search.fill('missing'); await expect(page.getByText('No matching projects.')).toBeVisible();
    await expect(label).toHaveText(name);
    await page.screenshot({ path: info.outputPath('filter-no-match.png') });
    await page.getByRole('button', { name: 'Clear search' }).click();
    if (mobile) await page.getByRole('button', { name: 'Close chat navigation' }).click();
    await page.getByRole('button', { name: 'Open project details', exact: true }).click();
    await expect(page.getByRole('button', { name: 'Close project details' })).toBeFocused();
    await page.getByRole('button', { name: 'Edit project', exact: true }).click();
    const editor = page.getByRole('form', { name: 'Edit project' });
    await editor.getByLabel('Project name', { exact: true }).fill('Discard this draft');
    ...
    await editor.getByLabel('Project name', { exact: true }).fill(updated.name);
    await editor.getByLabel('Project goal').fill(updated.goal);
    await editor.getByLabel('Repository reference').fill(updated.repository);
    await editor.getByLabel('Note references (one per line)').fill(updated.notes.join('\n'));
    ...
    await expect(editor.getByLabel('Project name', { exact: true })).toHaveValue(updated.name);
    ...
    await expect(page.getByRole('button', { name: 'Open project details' })).toBeFocused();
    await expect(page.getByLabel('Selected session', { exact: true })).toHaveText(title);
    ...
    if (mobile) await page.getByRole('button', { name: 'Close chat navigation' }).press('Escape');
    await expect(page.getByLabel('Selected session', { exact: true })).toHaveText(title);
    await expect(page.getByText('No saved messages in session history yet.', { exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('complementary', { name: 'Context workspace' })).toBeHidden();
