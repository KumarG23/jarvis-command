...
describe('Command session creation', () => {
  it('creates once with the exact mutation boundary, selects the validated session and adds navigation', async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      if (url.endsWith('/context')) return Promise.resolve(Response.json({ sessionId: created.id, state: 'unavailable', updatedAt: null, receipt: null }));
      if (url.includes('/messages?')) return Promise.resolve(Response.json(history(created.id, 0, [])));
